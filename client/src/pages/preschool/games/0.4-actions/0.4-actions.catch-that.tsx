...
const actionMap: Record<string, { bg: string; text: string; border: string }> = {
  "RUN": { bg: "bg-red-500", text: "text-white", border: "border-red-600" },
  "JUMP": { bg: "bg-green-500", text: "text-white", border: "border-green-600" },
  "EAT": { bg: "bg-orange-500", text: "text-white", border: "border-orange-600" },
  "DRINK": { bg: "bg-blue-500", text: "text-white", border: "border-blue-600" },
  "SLEEP": { bg: "bg-indigo-500", text: "text-white", border: "border-indigo-600" },
  "PLAY": { bg: "bg-yellow-400", text: "text-gray-800", border: "border-yellow-500" },
  "LISTEN": { bg: "bg-purple-500", text: "text-white", border: "border-purple-600" },
  "LOOK": { bg: "bg-cyan-500", text: "text-white", border: "border-cyan-600" },
  "SITDOWN": { bg: "bg-pink-500", text: "text-white", border: "border-pink-600" },
  "STANDUP": { bg: "bg-emerald-500", text: "text-white", border: "border-emerald-600" },
  "COMEHERE": { bg: "bg-rose-500", text: "text-white", border: "border-rose-600" },
  "BEQUIET": { bg: "bg-gray-500", text: "text-white", border: "border-gray-600" },
  "OPEN": { bg: "bg-lime-500", text: "text-gray-800", border: "border-lime-600" },
  "CLOSE": { bg: "bg-violet-500", text: "text-white", border: "border-violet-600" },
  "REPEAT": { bg: "bg-amber-500", text: "text-white", border: "border-amber-600" },
  "GO": { bg: "bg-teal-500", text: "text-white", border: "border-teal-600" },
  "STOP": { bg: "bg-red-600", text: "text-white", border: "border-red-700" },
};

// Function to convert single-word format back to spaced format for display
...
// Function to convert single-word format to spaced format for speech (lowercase)
const formatWordForSpeech = (word: string): string => {
  ...
  return wordMap[word] || word.toLowerCase();
};
...
export default function ActionsCatchGame() {
  ...
  const nextWord = useCallback(() => {
    ...
    // Clear all falling words when word changes
    setFallingWords([]);
    setFallingPrizes([]);
  ...
  const catchWord = useCallback((word: FallingWord) => {
    ...
    // Verify correctness against current word
  ...
  const shareGame = () => {
    const text = `I scored ${score} points in Catch That Actions! Can you beat my score? 🏃`;
    if (navigator.share) {
      navigator.share({ title: "Catch That Actions - English Learning Game", text, url: window.location.href });
    } else {
    ...
    <Layout>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col flex-1 min-h-0 p-2 sm:p-4">
          <PreschoolGameHeader 
            gameName="Catch That"
            description="Pre-School & 1st Grade - Theme: Actions"
          ...
          <div className={'flex-1 flex flex-col lg:flex-row items-stretch gap-4 lg:gap-8 bg-gradient-to-b from-amber-50 to-green-100/30 rounded-xl p-3 sm:p-6 ' + (isFullscreen ? 'min-h-[600px]' : 'min-h-[500px]')}>
            ...
            >
              <div className={'w-32 h-32 sm:w-48 sm:h-48 ' + (isFullscreen ? 'w-96 h-96 ' : '') + 'rounded-2xl bg-amber-50 flex items-center justify-center overflow-hidden mb-3 mx-auto'}>
                <img 
                  src={'/images/preschool/vocab/0.4-actions/' + currentWord.file} 
            ...
            <div className={'flex-1 relative overflow-hidden ' + (isFullscreen ? 'min-h-[600px]' : 'min-h-[500px]') + ' rounded-xl bg-gradient-to-b from-sky-200 to-blue-100 border-4 border-blue-200'}>
              ...
              {gameStarted && fallingWords.map((word) => {
                const wordStyle = actionMap[word.word];
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-6">
                  {gameWon 
                    ? `You caught all ${totalWords} actions!` 
