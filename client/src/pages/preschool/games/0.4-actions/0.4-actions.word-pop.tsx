...
function ActionBalloonShape({ word, shape }: { word: string, shape: string }) {
  const colorStyle = actionBalloonMap[word] || { color: "from-gray-400 to-gray-600", textColor: "text-white" };
...
export default function ActionsWordPopGame() {
  ...
  const shareGame = () => {
    const text = `I scored ${score} points in Word Pop Actions! Can you beat my score? 🎈`;
    if (navigator.share) {
      navigator.share({ title: "Word Pop Actions - English Learning Game", text, url: window.location.href });
    } else {
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-6">
                  {gameWon 
                    ? `You completed all ${totalWords} actions!` 
