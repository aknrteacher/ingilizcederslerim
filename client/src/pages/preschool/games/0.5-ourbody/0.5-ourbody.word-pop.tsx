...
const ourBodyBalloonMap: Record<string, { color: string, textColor: string }> = {
  "BODY": { color: "from-blue-400 to-blue-600", textColor: "text-white" },
  "HEAD": { color: "from-pink-400 to-pink-600", textColor: "text-white" },
  "ARMS": { color: "from-cyan-400 to-cyan-600", textColor: "text-white" },
  "LEGS": { color: "from-emerald-400 to-emerald-600", textColor: "text-white" },
  "FEET": { color: "from-indigo-400 to-indigo-600", textColor: "text-white" },
  "TOE": { color: "from-teal-400 to-teal-600", textColor: "text-white" },
  "HANDS": { color: "from-orange-400 to-orange-600", textColor: "text-white" },
  "FINGER": { color: "from-sky-400 to-sky-600", textColor: "text-white" },
  "HAIR": { color: "from-amber-400 to-amber-600", textColor: "text-white" },
  "FACE": { color: "from-fuchsia-400 to-fuchsia-600", textColor: "text-white" },
  "EARS": { color: "from-purple-400 to-purple-600", textColor: "text-white" },
  "NOSE": { color: "from-yellow-300 to-yellow-500", textColor: "text-gray-800" },
  "EYES": { color: "from-green-400 to-green-600", textColor: "text-white" },
  "MOUTH": { color: "from-red-400 to-red-600", textColor: "text-white" },
  "TEETH": { color: "from-slate-400 to-slate-600", textColor: "text-white" },
};
...
function OurBodyBalloonShape({ word, shape }: { word: string, shape: string }) {
  const colorStyle = ourBodyBalloonMap[word] || { color: "from-gray-400 to-gray-600", textColor: "text-white" };
...
export default function OurBodyWordPopGame() {
  ...
  const shareGame = () => {
    const text = `I scored ${score} points in Word Pop Our Body! Can you beat my score? 🎈`;
    if (navigator.share) {
      navigator.share({ title: "Word Pop Our Body - English Learning Game", text, url: window.location.href });
    } else {
    ...
    <Layout>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col flex-1 min-h-0 p-2 sm:p-4">
          <PreschoolGameHeader 
            gameName="Word Pop"
            description="Pre-School & 1st Grade - Theme: Our Body"
          ...
          <div className="flex-1 flex flex-col lg:flex-row items-stretch gap-4 lg:gap-8 bg-gradient-to-b from-amber-50 to-green-100/30 rounded-xl p-3 sm:p-6 min-h-0 overflow-hidden">
            ...
            >
              <div className="w-32 h-32 sm:w-48 sm:h-48 rounded-2xl bg-amber-50 flex items-center justify-center overflow-hidden mb-3 mx-auto">
                <img 
                  src={`/images/preschool/vocab/0.5-ourbody/${currentWord.file}`} 
                ...
                return (
                  ...
                  >
                    <div className="relative balloon-wiggle" style={{ animationDelay: `${index * 0.2}s` }}>
                      <OurBodyBalloonShape word={balloon.word} shape={balloon.shape} />
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-6">
                  {gameWon 
                    ? `You completed all ${totalWords} body parts!` 
