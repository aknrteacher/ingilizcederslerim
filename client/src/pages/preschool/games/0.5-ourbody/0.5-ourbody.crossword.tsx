...
export default function OurBodyCrosswordGame() {
  ...
  const collageImages = placedWords.map(pw => {
      ...
      return vocab?.file ? `/images/preschool/vocab/0.5-ourbody/${vocab.file}` : null;
  }).filter(Boolean) as string[];

  const shareGame = () => {
    const text = `I just solved the Our Body Word Cross! Can you beat it? 👋`;
    if (navigator.share) {
      navigator.share({
        title: "Word Cross - Our Body",
        text: text,
  ...
  const challengeFriend = () => {
    const text = `I challenge you to solve this Our Body Word Cross puzzle! 🏆`;
    if (navigator.share) {
      navigator.share({
