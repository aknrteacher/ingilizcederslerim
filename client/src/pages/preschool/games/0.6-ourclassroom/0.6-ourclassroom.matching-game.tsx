...
export default function OurClassroomMatchingGame() {
  ...
  const [selectedCards, setSelectedCards] = useState(selectRandomCards(7));
  ...
  useEffect(() => {
    const words: GameCard[] = selectedCards.map((item, idx) => ({
      id: `word-${idx}`,
      word: item.word,
      imageUrl: `/images/preschool/vocab/0.6-ourclassroom/${item.file}`,
  ...
  const resetGame = () => {
    ...
    const newCards = selectRandomCards(7);
  ...
  const shareGame = () => {
    const text = `I just scored ${formatTime(elapsedTime)} on Matchlings Our Classroom! Can you beat my time? 👋`;
    if (navigator.share) {
      navigator.share({
        title: "Matchlings",
        text: text,
  ...
  const challengeFriend = () => {
    const text = `Challenge me on Matchlings Our Classroom! Can you match all the classroom items faster than my ${formatTime(elapsedTime)}? 🏆`;
    if (navigator.share) {
      navigator.share({
