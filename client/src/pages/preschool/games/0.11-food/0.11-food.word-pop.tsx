...
const foodBalloonMap: Record<string, { color: string, textColor: string }> = {
...
function FoodBalloonShape({ word, shape }: { word: string, shape: string }) {
  const colorStyle = foodBalloonMap[word] || { color: "from-gray-400 to-gray-600", textColor: "text-white" };
...
export default function FoodWordPopGame() {
  ...
  const shareGame = () => {
    const text = `I scored ${score} points in Word Pop Food! Can you beat my score? 🎈`;
    if (navigator.share) {
      navigator.share({ title: "Word Pop Food - English Learning Game", text, url: window.location.href });
    } else {
  ...
  return (
    <Layout>
      <div 
        ...
        id="word-pop-food-game"
        data-testid="word-pop-food-game"
      >
        ...
        <div className="relative z-10 flex flex-col flex-1 min-h-0 p-2 sm:p-4">
          <PreschoolGameHeader 
            ...
            containerId="word-pop-food-game"
            icon="🎈"
          />
          ...
          <div className="flex-1 flex flex-col lg:flex-row items-stretch gap-4 lg:gap-8 bg-gradient-to-b from-amber-50 to-green-100/30 rounded-xl p-3 sm:p-6 min-h-0 overflow-hidden">
            ...
            >
              <div className="w-32 h-32 sm:w-48 sm:h-48 rounded-2xl bg-amber-50 flex items-center justify-center overflow-hidden mb-3 mx-auto">
                <img 
                  src={`/images/preschool/vocab/0.11-food/${currentWord.file}`} 
                ...
                return (
                  ...
                  >
                    <div className="relative balloon-wiggle" style={{ animationDelay: `${index * 0.2}s` }}>
                      <FoodBalloonShape word={balloon.word} shape={balloon.shape} />
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-6">
                  {gameWon 
                    ? `You completed all ${totalWords} foods!` 
