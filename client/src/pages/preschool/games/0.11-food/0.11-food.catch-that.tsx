...
const foodMap: Record<string, { bg: string; text: string; border: string }> = {
...
export default function FoodCatchGame() {
  ...
  }, [currentWord, wordsCompleted, usedWords, totalWords]);
  ...
  const shareGame = () => {
    const text = `I scored ${score} points in Catch That Food! Can you beat my score? 👋`;
    if (navigator.share) {
      navigator.share({ title: "Catch That Food - English Learning Game", text, url: window.location.href });
    } else {
      navigator.clipboard.writeText(text + " " + window.location.href);
    }
  };
          ...
          <div className={'flex-1 flex flex-col lg:flex-row items-stretch gap-4 lg:gap-8 bg-gradient-to-b from-amber-50 to-green-100/30 rounded-xl p-3 sm:p-6 ' + (isFullscreen ? 'min-h-[600px]' : 'min-h-[500px]')}>
            ...
            <div
              ...
              {gameStarted && (
                ...
                const wordStyle = foodMap[word.word];
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-6">
                  {gameWon 
                    ? `You caught all ${totalWords} foods!` 
