...
export default function ThingsMatchingGame() {
  const [, setLocation] = useLocation();
  const allVocabulary = [
    { word: "toy", file: "toys.png", turkish: "oyuncak" },
    ...
    { word: "puzzle", file: "puzzle.png", turkish: "yapboz" },
  ...
  useEffect(() => {
    const words: GameCard[] = selectedCards.map((item, idx) => ({
      id: `word-${idx}`,
      word: item.word,
      imageUrl: `/images/preschool/vocab/0.7-things/${item.file}`,
  ...
  const shareGame = () => {
    const text = `I just scored ${formatTime(elapsedTime)} on Matchlings Things! Can you beat my time? 👋`;
    if (navigator.share) {
      navigator.share({
        title: "Matchlings",
        text: text,
  ...
  const challengeFriend = () => {
    const text = `Challenge me on Matchlings Things! Can you match all the things faster than my ${formatTime(elapsedTime)}? 🏆`;
    if (navigator.share) {
      navigator.share({
