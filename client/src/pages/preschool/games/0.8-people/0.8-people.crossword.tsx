...
// Vocabulary from 0.8 (People)
const vocabulary = [
  { word: "TEACHER", clue: "öğretmen", file: "teacher.png" },
  { word: "FRIEND", clue: "arkadaş", file: "friend.png" },
  { word: "BOY", clue: "erkek çocuk", file: "boy.png" },
  { word: "GIRL", clue: "kız", file: "girl.png" },
  { word: "MOM", clue: "anne", file: "mom.png" },
  { word: "DAD", clue: "baba", file: "dad.png" },
...
export default function PeopleCrosswordGame() {
  ...
  const collageImages = placedWords.map(pw => {
      ...
      return vocab?.file ? `/images/preschool/vocab/0.8-people/${vocab.file}` : null;
  }).filter(Boolean) as string[];

  const shareGame = () => {
    const text = `I just solved the People Word Cross! Can you beat it? 👋`;
    if (navigator.share) {
      navigator.share({
        title: "Word Cross - People",
        text: text,
  ...
  const challengeFriend = () => {
    const text = `I challenge you to solve this People Word Cross puzzle! 🏆`;
    if (navigator.share) {
      navigator.share({
