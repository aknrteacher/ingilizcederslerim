...
const peopleMap: Record<string, { bg: string; text: string; border: string }> = {
  "TEACHER": { bg: "bg-blue-500", text: "text-white", border: "border-blue-600" },
  "FRIEND": { bg: "bg-green-500", text: "text-white", border: "border-green-600" },
  "BOY": { bg: "bg-purple-500", text: "text-white", border: "border-purple-600" },
  "GIRL": { bg: "bg-pink-500", text: "text-white", border: "border-pink-600" },
  "MOM": { bg: "bg-yellow-400", text: "text-gray-800", border: "border-yellow-500" },
  "DAD": { bg: "bg-orange-500", text: "text-white", border: "border-orange-600" },
  "SISTER": { bg: "bg-cyan-500", text: "text-white", border: "border-cyan-600" },
...
export default function PeopleCatchGame() {
  ...
  const shareGame = () => {
    const text = `I scored ${score} points in Catch That People! Can you beat my score? 👋`;
    if (navigator.share) {
      navigator.share({ title: "Catch That People - English Learning Game", text, url: window.location.href });
    } else {
          ...
          <div className={'flex-1 flex flex-col lg:flex-row items-stretch gap-4 lg:gap-8 bg-gradient-to-b from-amber-50 to-green-100/30 rounded-xl p-3 sm:p-6 ' + (isFullscreen ? 'min-h-[600px]' : 'min-h-[500px]')}>
            ...
            <div className={'flex-1 relative overflow-hidden ' + (isFullscreen ? 'min-h-[600px]' : 'min-h-[500px]') + ' rounded-xl bg-gradient-to-b from-sky-200 to-blue-100 border-4 border-blue-200'}>
              ...
              {gameStarted && fallingWords.map((word) => {
                const wordStyle = peopleMap[word.word];
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-6">
                  {gameWon 
                    ? `You caught all ${totalWords} people!` 
