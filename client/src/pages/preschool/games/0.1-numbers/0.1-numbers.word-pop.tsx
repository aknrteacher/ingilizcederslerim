...
const numberBalloonMap: Record<string, { color: string, textColor: string }> = {
  "ONE": { color: "from-red-400 to-red-600", textColor: "text-white" },
  "TWO": { color: "from-blue-400 to-blue-600", textColor: "text-white" },
  "THREE": { color: "from-yellow-300 to-yellow-500", textColor: "text-gray-800" },
  "FOUR": { color: "from-green-400 to-green-600", textColor: "text-white" },
  "FIVE": { color: "from-orange-400 to-orange-600", textColor: "text-white" },
  "SIX": { color: "from-purple-400 to-purple-600", textColor: "text-white" },
  "SEVEN": { color: "from-pink-400 to-pink-500", textColor: "text-white" },
  "EIGHT": { color: "from-cyan-400 to-cyan-600", textColor: "text-white" },
  "NINE": { color: "from-indigo-400 to-indigo-600", textColor: "text-white" },
  "TEN": { color: "from-amber-700 to-amber-900", textColor: "text-white" },
};
...
function NumberBalloonShape({ word, shape }: { word: string, shape: string }) {
  const colorStyle = numberBalloonMap[word] || { color: "from-gray-400 to-gray-600", textColor: "text-white" };
  ...
  // Each letter needs approximately 12-14px width for bold text, plus padding (20px each side)
  const minWidth = Math.max(128, word.length * 14 + 40); // Minimum 128px, or word length * 14 + padding
  const minHeight = Math.max(128, minWidth * 0.95); // Keep aspect ratio close to square
...
export default function NumbersWordPopGame() {
  ...
  const shareGame = () => {
    const text = `I scored ${score} points in Word Pop Numbers! Can you beat my score? 🎈`;
    if (navigator.share) {
      navigator.share({ title: "Word Pop Numbers - English Learning Game", text, url: window.location.href });
    } else {
            ...
            >
              ...
              >
                ...
                <p className="text-gray-600 mb-6">
                  {gameWon 
                    ? `You completed all ${totalWords} numbers!` 
