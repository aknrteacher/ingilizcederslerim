...
const aroundUsBalloonMap: Record<string, { color: string, textColor: string }> = {
  "ANIMALS": { color: "from-orange-400 to-orange-600", textColor: "text-white" },
  "CITY": { color: "from-purple-400 to-purple-600", textColor: "text-white" },
  "CLOUDS": { color: "from-white to-gray-200", textColor: "text-gray-800" },
  "FLOWER": { color: "from-pink-400 to-pink-600", textColor: "text-white" },
  "HOUSE": { color: "from-amber-500 to-amber-700", textColor: "text-white" },
  "GRASS": { color: "from-lime-400 to-lime-600", textColor: "text-white" },
  "MOON": { color: "from-gray-300 to-gray-500", textColor: "text-gray-800" },
  "PEOPLE": { color: "from-pink-400 to-pink-600", textColor: "text-white" },
  "RAIN": { color: "from-slate-400 to-slate-600", textColor: "text-white" },
  "SKY": { color: "from-cyan-400 to-blue-600", textColor: "text-white" },
  "STAR": { color: "from-indigo-400 to-indigo-600", textColor: "text-white" },
  "SUN": { color: "from-yellow-400 to-orange-600", textColor: "text-white" },
  "TREE": { color: "from-green-500 to-green-700", textColor: "text-white" },
  "WATER": { color: "from-blue-300 to-blue-500", textColor: "text-white" },
  "WORLD": { color: "from-blue-400 to-blue-600", textColor: "text-white" },
};
...
function AroundUsBalloonShape({ word, shape }: { word: string, shape: string }) {
  const colorStyle = aroundUsBalloonMap[word] || { color: "from-gray-400 to-gray-600", textColor: "text-white" };
...
export default function AroundUsWordPopGame() {
  ...
  const shareGame = () => {
    const text = `I scored ${score} points in Word Pop Around Us! Can you beat my score? 🎈`;
    if (navigator.share) {
      navigator.share({ title: "Word Pop Around Us - English Learning Game", text, url: window.location.href });
    } else {
          ...
          <div className="flex-1 flex flex-col lg:flex-row items-stretch gap-4 lg:gap-8 bg-gradient-to-b from-amber-50 to-green-100/30 rounded-xl p-3 sm:p-6 min-h-0 overflow-hidden">
            ...
            >
              <div className="w-32 h-32 sm:w-48 sm:h-48 rounded-2xl bg-amber-50 flex items-center justify-center overflow-hidden mb-3 mx-auto">
                <img 
                  src={`/images/preschool/vocab/0.10-aroundus/${currentWord.file}`} 
                ...
                return (
                  ...
                  >
                    <div className="relative balloon-wiggle" style={{ animationDelay: `${index * 0.2}s` }}>
                      <AroundUsBalloonShape word={balloon.word} shape={balloon.shape} />
