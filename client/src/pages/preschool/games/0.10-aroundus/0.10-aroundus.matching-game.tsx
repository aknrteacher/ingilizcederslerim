...
export default function AroundUsMatchingGame() {
  ...
  useEffect(() => {
    const words: GameCard[] = selectedCards.map((item, idx) => ({
      id: `word-${idx}`,
      word: item.word,
      imageUrl: `/images/preschool/vocab/0.10-aroundus/${item.file}`,
  ...
  const shareGame = () => {
    const text = `I just scored ${formatTime(elapsedTime)} on Matchlings Around Us! Can you beat my time? 👋`;
    if (navigator.share) {
      navigator.share({
        title: "Matchlings",
        text: text,
  ...
  const challengeFriend = () => {
    const text = `Challenge me on Matchlings Around Us! Can you match all the items faster than my ${formatTime(elapsedTime)}? 🏆`;
    if (navigator.share) {
      navigator.share({
