...
const vocabulary = [
  ...
  { word: "MOTHER", turkish: "Anne", file: "mother.png" },
  { word: "BROTHER", turkish: "Erkek kardeş", file: "brother.png" },
  { word: "SISTER", turkish: "Kız kardeş", file: "sister.png" },
  { word: "SON", turkish: "Oğul", file: "son.png" },
  { word: "DAUGHTER", turkish: "Kız", file: "daughter.png" },
  { word: "UNCLE", turkish: "Amca/dayı", file: "uncle.png" },
  { word: "AUNT", turkish: "Hala/teyze", file: "aunt.png" },
  { word: "GRANDFATHER", turkish: "Büyükbaba", file: "grandfather.png" },
  { word: "GRANDMOTHER", turkish: "Büyükanne", file: "grandmother.png" },
  { word: "COUSIN", turkish: "Kuzen", file: "cousin.png" },
  { word: "CHILDREN", turkish: "Çocuklar", file: "children.png" },
  { word: "GRANDCHILDREN", turkish: "Torunlar", file: "grandchildren.png" },
  { word: "WHO", turkish: "Kim", file: "who.png" },
  { word: "THIS", turkish: "Bu", file: "this.png" },
  { word: "THAT", turkish: "Şu/o", file: "that.png" },
  { word: "SAY", turkish: "Söylemek", file: "say.png" },
  { word: "STUDENT", turkish: "Öğrenci", file: "student.png" },
  { word: "AGE", turkish: "Yaş", file: "age.png" },
  { word: "HOWOLDISHE", turkish: "O kaç yaşında?", file: "how old is he.png" },
  { word: "YEARSOLD", turkish: "Yaşında", file: "years old.png" },
  { word: "MY", turkish: "Benim", file: "my.png" },
  { word: "NAME", turkish: "Isim", file: "name.png" },
  { word: "IS", turkish: "Dır/dir", file: "is.png" },
  { word: "HASGOT", turkish: "Sahip", file: "has got.png" },
  { word: "INTRODUCE", turkish: "Tanıtmak", file: "introduce.png" },
...
// Diverse vibrant colors for balloons - different from each other and from game's green theme
const balloonStyles = [
  { shape: "round", color: "#FF4444", pattern: "stripes" },      // Red
  { shape: "round", color: "#4444FF", pattern: "hearts" },       // Blue
  { shape: "oval", color: "#AA44FF", pattern: "circles" },        // Purple
  { shape: "oval", color: "#FFFF44", pattern: "waves" },          // Yellow
  { shape: "heart", color: "#FF44AA", pattern: "none" },         // Pink
  { shape: "star", color: "#00FFFF", pattern: "none" },          // Cyan
  { shape: "round", color: "#FF44FF", pattern: "dots" },         // Magenta
  { shape: "oval", color: "#FF8844", pattern: "stripes" },       // Orange
];
...
function BalloonShape({ style, word }: { style: typeof balloonStyles[0], word: string }) {
  const baseClasses = `shadow-xl flex items-center justify-center cursor-pointer relative overflow-visible`;
  const bgStyle = { backgroundColor: style.color };
  ...
  switch (style.shape) {
    case "heart":
      return (
        <div className={`w-32 h-32 ${baseClasses}`} style={{ ...bgStyle, clipPath: "path('M64 120 C20 80 0 40 32 20 C50 8 64 20 64 35 C64 20 78 8 96 20 C128 40 108 80 64 120Z')" }}>
          {patternOverlay()}
          {shineEffect}
        </div>
      );
    case "star":
      return (
        <div className={`w-36 h-36 ${baseClasses}`} style={{ ...bgStyle, clipPath: "polygon(50% 0%, 61% 35%, 98% 35%, 68% 57%, 79% 91%, 50% 70%, 21% 91%, 32% 57%, 2% 35%, 39% 35%)" }}>
          {patternOverlay()}
          {shineEffect}
        </div>
      );
    case "oval":
      return (
        <div className={`w-24 h-36 rounded-[50%] ${baseClasses}`} style={bgStyle}>
          {patternOverlay()}
          {shineEffect}
        </div>
      );
    default: // round
      return (
        <div className={`w-32 h-32 rounded-full ${baseClasses}`} style={bgStyle}>
          {patternOverlay()}
          {shineEffect}
        </div>
      );
...
export default function WordPopGame() {
  ...
  const nextWord = useCallback(() => {
    if (wordsCompleted + 1 >= totalWords) {
      ...
      confetti({
        ...
        spread: 100,
        origin: { y: 0.6 },
        colors: ["#44FF44", "#55FF55", "#66FF66", "#33FF33", "#22FF22"]
      });
      return;
    }
  ...
  return (
    <Layout>
      ...
      >
        {/* Background - Green theme for primary school */}
        <div className="absolute inset-0 bg-[#44FF44] z-0">
          <div className="absolute bottom-0 left-0 right-0 h-32 bg-[#55FF55] opacity-50" />
        ...
        <div className="relative z-10 flex flex-col flex-1 min-h-0 p-2 sm:p-4">
          ...
          <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto justify-between sm:justify-end flex-wrap mb-4 flex-shrink-0">
            ...
            <div className="bg-green-100 px-2 sm:px-3 py-1 rounded-lg flex items-center gap-1 sm:gap-2">
              <Star className="h-3 w-3 sm:h-4 sm:w-4 text-yellow-500 fill-yellow-500" />
              <span className="font-bold text-green-700 text-sm sm:text-base">{score}</span>
            </div>
          ...
          <AnimatePresence>
            {showCombo && combo >= 2 && (
              ...
              >
                <div className="bg-[#44FF44] text-white px-6 py-3 rounded-full font-bold text-2xl shadow-lg">
                  🔥 {combo}x Combo!
                </div>
              </motion.div>
            )}
          ...
          <div className="flex-1 flex flex-col lg:flex-row items-stretch gap-4 lg:gap-8 bg-[#EEFFEE] rounded-xl p-3 sm:p-6 min-h-0 overflow-hidden">
            {/* Target Word Card */}
            <motion.div 
              key={currentWord.word}
              initial={{ scale: 0.8, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              className="bg-white rounded-3xl shadow-xl p-3 sm:p-4 border-4 border-[#44FF44] flex-shrink-0 self-center w-full lg:w-auto"
            >
              <div className="w-32 h-32 sm:w-48 sm:h-48 rounded-2xl bg-green-50 flex items-center justify-center overflow-hidden mb-3 mx-auto">
                <img 
                  src={`/images/primary/3.2/${currentWord.file}`} 
            ...
            <div className="flex-1 relative overflow-visible min-h-[300px]">
              {!gameStarted && !gameOver && !gameWon && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <Button
                    onClick={startGame}
                    size="lg"
                    className="bg-[#44FF44] hover:bg-[#33FF33] text-white text-xl px-8 py-6 rounded-2xl shadow-xl"
                  >
                    🎈 Start Game!
                  </Button>
                </div>
              ...
              {gameStarted && balloons.map((balloon, index) => {
                const balloonColor = balloonStyles[balloon.styleIndex].color;
                const bannerStyle = {
          ...
          <div className="primary-school-game-footer flex-shrink-0 mt-auto">
            <div className="footer-content">
              ...
              <div className="footer-right">
                ...
                <Button variant="outline" className="footer-button" onClick={() => setLocation("/primary-school/grade-3/unit-2/games")}>
                  ← Back
                </Button>
              </div>
            </div>
              ...
              >
                ...
                <div className="bg-green-50 rounded-xl p-4 mb-6">
                  <div className="flex items-center justify-center gap-2 mb-3">
                    ...
                    <span className="text-2xl font-bold text-green-700">{score} points</span>
                  </div>
                  <div className="text-sm space-y-1 border-t border-gray-200 pt-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-green-600">{wordsCompleted - hintsUsed} × {noHintBonus} = +{Math.max(0, (wordsCompleted - hintsUsed)) * noHintBonus}</span>
                    </div>
                    {hintsUsed > 0 && (
                      <div className="flex justify-between">
                ...
                <div className="flex flex-col gap-3">
                  <Button
                    onClick={resetGame}
                    size="lg"
                    className="w-full bg-[#44FF44] hover:bg-[#33FF33] text-white rounded-xl"
                  >
                    Play Again
                  </Button>
                  <Button
