...
export default function MatchingGame() {
  ...
  useEffect(() => {
    const words: GameCard[] = selectedCards.map((item, idx) => ({
      id: `word-${idx}`,
      word: item.word,
      imageUrl: `/images/primary/3.10/${item.file}`,
  ...
  const shareGame = () => {
    const text = `I just scored ${formatTime(elapsedTime)} on Matchlings Unit 10! Can you beat my time? 🎮`;
    if (navigator.share) {
      navigator.share({
        title: "Matchlings",
        text: text,
  ...
  const challengeFriend = () => {
    const text = `Challenge me on Matchlings Unit 10! Can you match all the words faster than my ${formatTime(elapsedTime)}? 🏆`;
    if (navigator.share) {
      navigator.share({
                  ...
                  <div className="win-modal">
                    <div className="win-content">
                      ...
                      <div className="win-buttons">
                        ...
                        <Button variant="ghost" onClick={() => setLocation("/primary-school/grade-3/unit-10/games")} className="btn-secondary">
                          Back to Games
                        </Button>
                      </div>
                    </div>
