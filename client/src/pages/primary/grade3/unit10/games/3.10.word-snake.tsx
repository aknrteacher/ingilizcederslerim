...
export default function WordSnakeGame() {
  ...
  /** A–Z only — used for snake tiles and progress (ignores spaces/punctuation in phrases). */
  ...
  const setupWord = useCallback(() => {
    ...
    const word = vocab.word.toLowerCase().replace(/[^a-z]/g, "");
    setTargetSpelling(word);
    setTargetWord(vocab);
    setCollectedLetters([]);
  ...
  useEffect(() => {
    ...
    if (hitLetter) {
      const nextNeededLetter = targetSpelling[collectedLetters.length]?.toUpperCase();
      ...
      if (hitLetter.char === nextNeededLetter) {
        ...
        if (collectedLetters.length + 1 === targetSpelling.length) {
  ...
  }, [snake, letters, targetWord, targetSpelling, collectedLetters, gameStarted, gameOver, wordsCompleted, setupWord, speakWord, speakLetter, playBuzzSound, getRandomPosition]);
          ...
          {gameStarted && !gameOver && (
            <>
                ...
                {targetWord && (
                  <div className="target-overlay">
                    <div className="word-progress-compact">
                      {targetSpelling.split('').map((char, idx) => (
                        <span 
                          key={idx} 
              ...
              <div 
                className="game-grid"
                style={{
                  backgroundImage: targetWord ? `url(/images/primary/3.10/${targetWord.file})` : 'none'
                }}
              >
                {/* Snake */}
