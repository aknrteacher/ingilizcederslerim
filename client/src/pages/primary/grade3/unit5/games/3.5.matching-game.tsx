...
import "@/styles/3.5.matching-game.css";
...
export default function MatchingGame() {
  ...
  const allVocabulary = [
    ...
    { word: "your", file: "your.png", turkish: "senin" },
  ...
  useEffect(() => {
    const words: GameCard[] = selectedCards.map((item, idx) => ({
      id: `word-${idx}`,
      word: item.word,
      imageUrl: `/images/primary/3.5/${item.file}`,
  ...
  const shareGame = () => {
    const text = `I just scored ${formatTime(elapsedTime)} on Matchlings Unit 5! Can you beat my time? 🎮`;
    if (navigator.share) {
      navigator.share({
        title: "Matchlings",
        text: text,
  ...
  const challengeFriend = () => {
    const text = `Challenge me on Matchlings Unit 5! Can you match all the words faster than my ${formatTime(elapsedTime)}? 🏆`;
    if (navigator.share) {
      navigator.share({
                  ...
                  <div className="win-modal">
                    <div className="win-content">
                      ...
                      <div className="win-buttons">
                        ...
                        <Button variant="ghost" onClick={() => setLocation("/primary-school/grade-3/unit-5/games")} className="btn-secondary">
                          Back to Games
                        </Button>
                      </div>
                    </div>
