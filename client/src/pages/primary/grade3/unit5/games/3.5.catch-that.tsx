...
import "@/styles/3.5.catch-that.css";
...
// Unit 5 vocabulary - uppercase single words for catch-that
const vocabulary = [
...
export default function CatchThatGame3_5() {
  ...
  const shareGame = () => {
    const text = `I scored ${score} points in Catch That Unit 5! Can you beat my score? 🎯`;
    if (navigator.share) {
      navigator.share({ title: "Catch That - Unit 5 English Learning Game", text, url: window.location.href });
    } else {
      navigator.clipboard.writeText(text + " " + window.location.href);
    }
  };
        ...
        <div className="relative z-10 flex flex-col flex-1 min-h-0 p-2 sm:p-4">
          ...
          <div className={'flex-1 flex flex-col lg:flex-row items-stretch gap-4 lg:gap-8 bg-[#FFEEEE] rounded-xl p-3 sm:p-6 ' + (isFullscreen ? 'min-h-[600px]' : 'min-h-[500px]')}>
            ...
            <motion.div 
              ...
              className={'bg-white rounded-3xl shadow-xl p-3 sm:p-4 ' + (isFullscreen ? 'p-6 ' : '') + 'border-4 border-[#FF66AA] flex-shrink-0 self-center w-full lg:w-auto '}
            >
            ...
            <div
              ref={playAreaRef}
              className={'flex-1 relative overflow-hidden touch-none ' + (isFullscreen ? 'min-h-[600px]' : 'min-h-[500px]') + ' rounded-xl bg-[#FFEEEE] border-4 border-[#FF66AA]'}>
              {!gameStarted && !gameOver && !gameWon && (
