...
import "@/styles/3.5.spell-quest.css";
...
// Unit 5 vocabulary - uppercase single words for spelling (multi-word phrases converted to single words)
...
export default function SpellQuestGame() {
  ...
  const shareGame = () => {
    const text = `I scored ${score} points on Spell Quest Unit 5! Can you beat my score? ✨`;
    if (navigator.share) {
      navigator.share({ title: "Spell Quest Unit 5", text, url: window.location.href });
    } else {
      alert(text);
    }
  };
  ...
  const challengeFriend = () => {
    const text = `Challenge me on Spell Quest Unit 5! I scored ${score} points - can you do better? 🏆`;
    if (navigator.share) {
      navigator.share({ title: "Spell Quest Challenge Unit 5", text, url: window.location.href });
    } else {
      alert(text);
    }
  };
          ...
          {gameComplete && (
            ...
            >
              <div className="complete-content">
                ...
                <Button variant="outline" onClick={() => setLocation("/primary-school/grade-3/unit-5/games")}>
                  Back to Games
                </Button>
                </div>
              </div>
