...
// Vocabulary from 2.4 (Family Life) - convert multi-word to single word for crossword
const vocabulary = [
  ...
  { word: "GRANDCHILD", clue: "Torun", file: "grandchild.png" },
  { word: "CHILD", clue: "Çocuk", file: "child.png" },
  { word: "BABY", clue: "Bebek", file: "baby.png" },
  { word: "MAN", clue: "Erkek", file: "man.png" },
  ...
  { word: "LOVE", clue: "Sevmek", file: "love.png" },
...
export default function CrosswordGame2_4() {
  ...
  const collageImages = placedWords.map(pw => {
      const vocab = vocabulary.find(v => v.word === pw.word);
              return vocab?.file ? `/images/primary/2.4/${vocab.file}` : null;
  }).filter(Boolean) as string[];

  const shareGame = () => {
    const text = `I just solved the Family Life Word Cross! Can you beat it? 🧩`;
    if (navigator.share) {
      navigator.share({
        title: "Word Cross - Family Life",
        text: text,
  ...
  const challengeFriend = () => {
    const text = `I challenge you to solve this Family Life Word Cross puzzle! 🏆`;
    if (navigator.share) {
      navigator.share({
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <Button onClick={() => setLocation("/primary-school/grade-2/theme-4/games")} variant="ghost" size="lg" className="w-full text-slate-600 hover:bg-slate-50 rounded-xl">
                      Back to Games
                    </Button>
                  </div>
                </motion.div>
