/**
 * Applies Catch That gameplay fixes to Primary (grades 2–4) variants:
 * play-area basket coords, stable RAF via basketPositionRef, immediate round advance,
 * combo scoring alignment, modal honesty, Challenge button.
 * Does NOT change getSpawnInterval / getFallSpeed (pace tiers stay as authored).
 *
 * Grade 2 only: imports speakCatchThatTargetWord + auto-announces target each round.
 *
 * Usage (from repo root): node scripts/patch-primary-catch-that.mjs
 */
import fs from "fs";
...
function patch(content, relPath) {
  ...
  const isG2 = relPath.includes(`${path.sep}grade2${path.sep}`);

  if (isG2 && !c.includes("@/lib/catch-that-speech")) {
    c = c.replace(
  ...
  if (isG2 && !c.includes("speakCatchThatTargetWord(phrase)")) {
    c = c.replace(
  ...
  if (!c.includes("handleTouchStart")) {
    ...
    c = c.replace(/gameArea\.addEventListener\('wheel'/g, "playArea.addEventListener('wheel'");
    c = c.replace(/gameArea\.addEventListener\('mousedown'/g, "playArea.addEventListener('mousedown'");
    c = c.replace(/gameArea\.addEventListener\('mouseup'/g, "playArea.addEventListener('mouseup'");
  ...
  c = c.replace(/, basketPosition, catchWord,/g, ", catchWord,");
  ...
  c = c.replace(/\{\/\* Game Play Area \*\/\}\n            <div className=\{'flex-1 relative overflow-hidden '/g, `{/* Game Play Area — basket coords use this region only */}
            <div
              ref={playAreaRef}
              className={'flex-1 relative overflow-hidden touch-none '`);

  c = c.replace(
    /\{gameStarted && !gameOver && !gameWon && \(\s*\n                <div className="absolute top-2[^"]*"[^>]*>\s*\n                  [^<]*<\/div>\s*\n              \)\}/,
...
const primaryRoots = [
  path.join(ROOT, "client/src/pages/primary/grade2"),
  path.join(ROOT, "client/src/pages/primary/grade3"),
  path.join(ROOT, "client/src/pages/primary/grade4"),
];

for (const dir of primaryRoots) {
  const files = walkCatchThat(dir);
  for (const fp of files) {
    const before = fs.readFileSync(fp, "utf8");
    const after = patch(before, path.relative(ROOT, fp));
    if (after !== before) {
